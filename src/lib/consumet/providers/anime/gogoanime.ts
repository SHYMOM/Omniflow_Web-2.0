...
class Gogoanime extends AnimeParser {
  ...
  override search = async (query: string, page: number = 1): Promise<ISearch<IAnimeResult>> => {
    ...
    try {
      const isAniNeko = this.baseUrl.includes('anineko.to');
      const searchUrl = isAniNeko
      ...
      if (isAniNeko) {
        searchResult.hasNextPage = $('.nv-browse-grid').length > 0;
        $('.nv-browse-grid > article').each((i, el) => {
          const titleA = $(el).find('h3.nv-anime-title a');
          const img = $(el).find('a.nv-anime-thumb img');
          ...
          searchResult.results.push({
            ...
            url: href.startsWith('http') ? href : `${this.baseUrl}${href.startsWith('/') ? href.substring(1) : href}`,
            image: img.attr('src') || '',
            subOrDub: titleA.text().toLowerCase().includes('dub') ? SubOrSub.DUB : SubOrSub.SUB,
          });
        });
      } else {
  ...
  override fetchAnimeInfo = async (id: string): Promise<IAnimeInfo> => {
    const isAniNeko = this.baseUrl.includes('anineko.to') || id.includes('watch/');
    let targetUrl = id;
    if (isAniNeko) {
      if (!id.startsWith('http')) {
        targetUrl = `${this.baseUrl}${id.startsWith('watch/') ? '' : 'watch/'}${id}`;
      }
    } else {
      if (!id.includes('gogoanime') && !id.startsWith('http')) {
        targetUrl = `${this.baseUrl}/category/${id}`;
      }
    }
    ...
    try {
      ...
      if (isAniNeko) {
        animeInfo.id = targetUrl.replace(this.baseUrl, '').replace(/^watch\//, '').replace(/^\//, '');
        animeInfo.title = $('.nv-anime-title').text().trim() || $('h1').first().text().trim();
        animeInfo.url = targetUrl;
        animeInfo.image = $('.nv-anime-thumb img').attr('src') || $('img.cover').attr('src') || '';
        animeInfo.description = $('.nv-anime-description').text().trim() || $('meta[name="description"]').attr('content') || '';
        ...
        const parts = animeInfo.id.split('/');
        const cleanId = parts[parts.length - 1];

        $('a').each((i, el) => {
          const href = $(el).attr('href') || '';
          if (href.includes(`/watch/${cleanId}/ep-`)) {
            const epParts = href.split('/ep-');
            const epNumStr = epParts[epParts.length - 1];
            const epNum = parseInt(epNumStr, 10);
            if (!isNaN(epNum)) {
              const text = $(el).text().trim().replace(/\s+/g, ' ');
              const existing = episodesMap.get(epNum);
              const isBetterTitle = !existing ||
                (existing.title === 'Watch Now' || existing.title === 'Watch' || (text !== 'Watch Now' && text !== 'Watch' && text.length > existing.title.length));
        ...
        animeInfo.episodes = Array.from(episodesMap.values()).sort((a, b) => a.number - b.number);
        animeInfo.totalEpisodes = animeInfo.episodes.length;
      } else {
        animeInfo.id = new URL(targetUrl).pathname.split('/')[2];
        animeInfo.title = $(
  ...
  override fetchEpisodeSources = async (
    ...
    if (episodeId.startsWith('http')) {
      ...
      // Handle direct VibePlayer link extracted from AniNeko layout
      if (episodeId.includes('vibeplayer.site')) {
        const match = episodeId.match(/vibeplayer\.site\/([a-zA-Z0-9]+)/);
        if (match) {
          const fileUrl = `https://vibeplayer.site/public/stream/${match[1]}/master.m3u8`;
          const subUrl = serverUrl.searchParams.get('sub');
          return {
            headers: {
            ...
            download: downloadUrl ? downloadUrl : fileUrl,
          };
        }
      }
    ...
    try {
      const isAniNeko = this.baseUrl.includes('anineko.to') || episodeId.includes('watch/');
      const targetUrl = episodeId.startsWith('http')
        ? episodeId
        : `${this.baseUrl}${episodeId.startsWith('/') ? episodeId.substring(1) : episodeId}`;
      ...
      if (isAniNeko) {
        let matchedUrl = '';
        const getLink = (el: any) => $(el).attr('data-id') || $(el).attr('data-video') || $(el).attr('data-url') || $(el).attr('data-src') || '';

        if (server === StreamingServers.GogoCDN || server === StreamingServers.VidStreaming) {
          $('button, a').each((i, el) => {
            const link = getLink(el);
            if (link.includes('vibeplayer.site')) {
              matchedUrl = link;
              return false;
            }
          });
          ...
          $('button, a').each((i, el) => {
            const link = getLink(el);
            if (link.includes('otakuhg.site') || link.includes('streamwish') || link.includes('otakuvid.online')) {
              matchedUrl = link;
              return false;
            }
          });
        ...
        if (!matchedUrl) {
          throw new Error('No streaming servers found on this page.');
        }

        return await this.fetchEpisodeSources(matchedUrl, server);
      } else {
        let serverUrl: URL;
