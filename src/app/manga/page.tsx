...
import { getPopularManga } from '@/lib/api/anilist';
...
export default function MangaLandingPage() {
  ...
  async function loadMore() {
    ...
    setLoading(true);
    try {
      const res = await getPopularManga(24, page);
      const newItems = res.map(mapAniListToMediaItem);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 pt-20">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-white mb-2 font-display">Manga Features</h1>
        <p className="text-xs text-text-secondary">Explore spectacular illustrated series across all popular genres</p>
      </div>
