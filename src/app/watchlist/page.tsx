        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
          {(() => {
            ...
            return watchlist.map((entry) => {
              if (seen.has(entry.mediaId)) return null;
              seen.add(entry.mediaId);

              const safeTitle = typeof entry.title === 'string' ? entry.title : ((entry.title as any)?.english || (entry.title as any)?.romaji || 'Unknown Title');
              const cleanId = entry.mediaId.replace(/^(anilist-|tmdb-movie-|tmdb-tv-)/, '');
              
              return (
                <div key={entry.mediaId} className="relative group">
                  <Link href={`/${entry.mediaType}/${cleanId}`}>
                    <div className="relative aspect-[3/4] rounded-lg overflow-hidden bg-surface mb-2">
                      <Image src={entry.posterUrl} alt={safeTitle} fill className="object-cover group-hover:scale-105 transition-transform" />
                      {entry.score > 0 && (
                    ...
                    <p className="text-[13px] text-white line-clamp-1">{safeTitle}</p>
                  </Link>
